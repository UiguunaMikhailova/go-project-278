...
`original_url` - обязательное поле, должно быть URL.
`short_name` - необязательное, от 3 до 32 символов, уникальное.
Если поле не передали, сервер сгенерирует имя сам; `created_at` проставляет база.

Ошибки валидации - 422 с описанием по каждому полю:

```json
{ "errors": { "short_name": "short name already in use" } }
```

Нечитаемый JSON - 400 с телом `{"error": "invalid request"}`.

`GET` `/r/:code` - 302 редирект на original_url - 404 если ссылка не найдена
`GET` `/api/link_visits` - 200 список посещений
...
Диапазон можно передать и заголовком `Range: [0,10]`.
В ответе приходит заголовок `Content-Range: links 0-10/42`, где последнее число - всего записей.
Для посещений единица измерения своя: `Content-Range: link_visits 0-10/357`.
